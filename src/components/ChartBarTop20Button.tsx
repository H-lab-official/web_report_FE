...
const ButtonChart: React.FC = () => {
    const [buttonClickData, setButtonClickData] = useState<CountContentData[]>([]);
    const [filteredData, setFilteredData] = useState<CountContentData[]>([]);
    const [totalButtonClickData, setTotalButtonClickData] = useState<CountContentData[]>([]); // เก็บข้อมูลทั้งหมดไว้
        ...
        const fetchLogContentData = async () => {
            try {
                ...
                if (response.data.totalCounts && Array.isArray(response.data.totalCounts)) {
                    const filteredButtons = response.data.totalCounts
                        .filter((item: CountContentData) => item.log_content && item.log_content.endsWith('_button')) // กรองเฉพาะที่ log_content ลงท้ายด้วย _button
                        .sort((a: any, b: any) => b.total_count! - a.total_count!) // เรียงข้อมูลจากมากไปน้อย
                        .slice(0, 20); // เลือกเฉพาะ Top 20

                    setButtonClickData(filteredButtons); // ตั้งค่าเริ่มต้นเป็นข้อมูลทั้งหมด
                    setTotalButtonClickData(filteredButtons); // เก็บข้อมูลทั้งหมดในตัวแปรนี้
                } else {
                ...
                if (response.data.countsByDate && Array.isArray(response.data.countsByDate)) {
                    const filteredButtonsByDate = response.data.countsByDate
                        .filter((item: CountContentData) => item.log_content && item.log_content.endsWith('_button'));
                    setFilteredData(filteredButtonsByDate);
                }
            } catch (error) {
    ...
    const resetDates = () => {
        ...
        setShowTotal(true);
        setButtonClickData(totalButtonClickData); // คืนค่าข้อมูลทั้งหมด
    };

    // Prepare data for chart
    const chartData = {
        labels: buttonClickData.map((data) => data.log_content), // Top 20 labels
        datasets: [
            {
                label: showTotal ? "Top 20 Button Clicks (All Time)" : "Top 20 Button Clicks (Filtered by Date)",
                data: buttonClickData.map((data) => showTotal ? data.total_count : data.count), // Top 20 counts
    ...
    return (
        <div className="p-4">
            <div className="flex flex-row  justify-center items-center gap-5">
                <h2 className="text-lg font-semibold">Top 20 Button Clicks</h2>
                <button
                    onClick={resetDates} // รีเซ็ตวันที่เมื่อกดปุ่ม "แสดงทั้งหมด"
